...
class PluginTest extends JUnit3Suite with ShouldMatchers with PluginMatchers with BeforeAndAfterAll {
  ...
  def testDefault {
    ...
    config should containCompoundArgs("-R", testOutputDirectory, outputDirectory)
  ...
  def testRunpath {
    configure(_.runpath = comma("http://foo.com/my.jar", "/some/where")) should containCompoundArgs("-R", testOutputDirectory, outputDirectory, "http://foo.com/my.jar", "/some/where")
  }
